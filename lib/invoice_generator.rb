  ...
  def run
    ...
    DB.transaction do
      active_billing_records.group_by { |br| br[:project] }.each do |project, project_records|
        ...
        project_content[:invoice_number] = "#{invoice_month}-#{invoice_customer}-#{invoice_order}"
        ...
        if @save_result
          Invoice.create_with_id(project_id: project.id, content: project_content)
          if credit_used > 0
            ...
            pr = Project[project.id]
            pr.credit = Sequel[:credit] - credit_used
            pr.save_changes(validate: false)
          end
        end
      end
    end
