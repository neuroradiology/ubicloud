...
RSpec.describe Prog::InstallNetworkingTools do
  ...
  describe "#start" do
    it "starts sub-programs to install dependencies and download dnsmasq concurrently" do
      ...
      expect(idm).to receive(:bud).with(described_class, {sshable_id: "bogus"}, :git_clone_radvd)
  ...
  describe "#wait_downloads" do
    ...
    it "hops to compile_and_install when the downloads are done" do
      ...
      expect(idm).to receive(:bud).with(described_class, {sshable_id: "bogus"}, :compile_and_install_dnsmasq)
      expect(idm).to receive(:bud).with(described_class, {sshable_id: "bogus"}, :compile_and_install_radvd)
...
CMD
  ...
  describe "#git_clone_radvd" do
    it "fetches a version and pops" do
      sshable = instance_double(Sshable)
      expect(sshable).to receive(:cmd).with <<CMD.rstrip
git init radvd && (cd radvd &&   git fetch https://github.com/furkansahin/radvd.git f85392a68c7cd0fe5525b4218be07b893402b69b --depth=1 &&  git checkout f85392a68c7cd0fe5525b4218be07b893402b69b &&  git fsck --full)
CMD
      expect(idm).to receive(:sshable).and_return(sshable)
